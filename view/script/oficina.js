...
function init(){
	...
	$('#nombre_oficina').validacion(' abcdefghijklmnñopqrstuvwxyzáéíóú0123456789/-*,.°()$#');
    $('#direccion_oficina').validacion(' abcdefghijklmnñopqrstuvwxyzáéíóú0123456789/-*,.°()$#');
    $('#oficina').select2();
...
function mostrar(id_oficina)
{
	$.post("../ajax/oficina.php?op=4",{id_oficina : id_oficina}, function(data, status)
	{
       ...
       $.post("../ajax/oficina.php?op=5", function(r){
            $("#oficina").html(r);
            $('select[name=correo_personal]').val(data.correo_personal);
            $('#oficina').trigger('change.select2');
        });
